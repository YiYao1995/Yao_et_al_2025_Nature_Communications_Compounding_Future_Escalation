...
CONTAINS 
  ...
  subroutine depvel_compute( bounds, &
    ...
    use pftconMod      , only : nc3irrig_dri, nc3irrig_spr, nc3irrig_fld, npcropmin, npcropmax
    use clm_varcon     , only : spval
    ...
    associate(                                                    & 
      ...
      pft_loop: do pi = bounds%begp,bounds%endp
         ...
         active: if (patch%active(pi)) then
            ...
            if (clmveg == nc3irrig_dri                            ) wesveg = 2
			if (clmveg == nc3irrig_spr                            ) wesveg = 2
			if (clmveg == nc3irrig_fld                            ) wesveg = 2

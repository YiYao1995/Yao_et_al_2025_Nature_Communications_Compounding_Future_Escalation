...
contains
  ...
  subroutine CropPhenology(num_pcropp, filter_pcropp                     , &
    ...
    use pftconMod        , only : nirrig_dri_tmp_corn, nirrig_dri_swheat, nirrig_dri_wwheat, nirrig_dri_tmp_soybean, nirrig_spr_tmp_corn, nirrig_spr_swheat, nirrig_spr_wwheat, nirrig_spr_tmp_soybean, nirrig_fld_tmp_corn, nirrig_fld_swheat, nirrig_fld_wwheat, nirrig_fld_tmp_soybean
    use pftconMod        , only : ntrp_corn, nsugarcane, ntrp_soybean, ncotton, nrice
    use pftconMod        , only : nirrig_dri_trp_corn, nirrig_dri_sugarcane, nirrig_dri_trp_soybean, nirrig_spr_trp_corn, nirrig_spr_sugarcane, nirrig_spr_trp_soybean, nirrig_fld_trp_corn, nirrig_fld_sugarcane, nirrig_fld_trp_soybean
    use pftconMod        , only : nirrig_dri_cotton, nirrig_dri_rice, nirrig_spr_cotton, nirrig_spr_rice, nirrig_fld_cotton, nirrig_fld_rice
    ...
    associate(                                                                   & 
      ...
      do fp = 1, num_pcropp
         ...
         if ( jday == jdayyrstart(h) .and. mcsec == 0 )then
            ...
            else if (croplive(p) .and. (ivt(p) == nwwheat .or. ivt(p) == nirrig_dri_wwheat .or. ivt(p) == nirrig_spr_wwheat .or. ivt(p) == nirrig_fld_wwheat)) then
               cropplant(p) = .false.
         ...
         if ( (.not. croplive(p)) .and. (.not. cropplant(p)) ) then
            ...
            if (ivt(p) == nwwheat .or. ivt(p) == nirrig_dri_wwheat .or. ivt(p) == nirrig_spr_wwheat .or. ivt(p) == nirrig_fld_wwheat) then
               ...
               if (t10(p) /= spval.and. a10tmin(p) /= spval   .and. &
                  ...
                  ! climatological date
                  if (ivt(p) == ntmp_soybean .or. ivt(p) == nirrig_dri_tmp_soybean .or. ivt(p) == nirrig_spr_tmp_soybean .or. ivt(p) == nirrig_fld_tmp_soybean .or. &
                       ivt(p) == ntrp_soybean .or. ivt(p) == nirrig_dri_trp_soybean .or. ivt(p) == nirrig_spr_trp_soybean .or. ivt(p) == nirrig_fld_trp_soybean) then
                  ...
                  if (ivt(p) == nswheat .or. ivt(p) == nirrig_dri_swheat .or. ivt(p) == nirrig_spr_swheat .or. ivt(p) == nirrig_fld_swheat .or. &
                      ivt(p) == ncotton .or. ivt(p) == nirrig_dri_cotton .or. ivt(p) == nirrig_spr_cotton .or. ivt(p) == nirrig_fld_cotton .or. &
                      ivt(p) == nrice   .or. ivt(p) == nirrig_dri_rice .or. ivt(p) == nirrig_spr_rice .or. ivt(p) == nirrig_fld_rice) then
         ...
         if (croplive(p)) then
            ...
            if (t_ref2m_min(p) < 1.e30_r8 .and. vf(p) /= 1._r8 .and. &
               (ivt(p) == nwwheat .or. ivt(p) == nirrig_dri_wwheat .or. ivt(p) == nirrig_spr_wwheat .or. ivt(p) == nirrig_fld_wwheat)) then

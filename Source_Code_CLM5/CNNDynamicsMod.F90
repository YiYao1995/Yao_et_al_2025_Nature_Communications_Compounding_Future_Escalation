      ...
      do fp = 1,num_soilp
         ...
         if (croplive(p) .and. &
              (patch%itype(p) == ntmp_soybean .or. &
               patch%itype(p) == nirrig_dri_tmp_soybean .or. &
			   patch%itype(p) == nirrig_spr_tmp_soybean .or. &
			   patch%itype(p) == nirrig_fld_tmp_soybean .or. &
               patch%itype(p) == ntrp_soybean .or. &
			   patch%itype(p) == nirrig_dri_trp_soybean .or. &
			   patch%itype(p) == nirrig_spr_trp_soybean .or. &
               patch%itype(p) == nirrig_fld_trp_soybean) ) then

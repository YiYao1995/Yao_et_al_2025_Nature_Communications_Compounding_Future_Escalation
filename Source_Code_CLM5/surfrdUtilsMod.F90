...
!-----------------------------------------------------------------------
  ...
  subroutine collapse_crop_types(wt_cft, fert_cft, cftsize, begg, endg, verbose, sumto)
    ...
    use pftconMod  , only : nc3crop, nc3irrig_dri, nc3irrig_spr, nc3irrig_fld, pftcon
    !
    ! !ARGUMENTS:
    ...
    if (cftsize > 0) then  ! The opposite applies only if use_fates
       ...
       if (.not. irrigate) then
          ...
          do g = begg, endg
             ...
             wt_cft(g, nc3crop:maxveg-3:4) = &
                  wt_cft(g, nc3crop:maxveg-3:4) + wt_cft(g, nc3irrig_dri:maxveg-2:4) + wt_cft(g, nc3irrig_spr:maxveg-1:4) + wt_cft(g, nc3irrig_fld:maxveg:4)
             wt_cft(g, nc3irrig_dri:maxveg-2:4)  = 0._r8
			 wt_cft(g, nc3irrig_spr:maxveg-1:4)  = 0._r8
			 wt_cft(g, nc3irrig_fld:maxveg:4)  = 0._r8
          end do

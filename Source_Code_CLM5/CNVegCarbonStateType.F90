...
#include "shr_assert.h"
  ...
  use pftconMod	     , only : noveg, npcropmin, pftcon, nc3crop, nc3irrig_dri, nc3irrig_spr, nc3irrig_fld
       ...
       if (  flag == 'read' .and. (enter_spinup .or. (reseed_dead_plants .and. .not. is_restart())) .and. .not. use_cndv) then
             ...
             do i = bounds%begp,bounds%endp
                if (this%totvegc_patch(i) .le. totvegcthresh) then
                   ...
                   l = patch%landunit(i)
                   if (lun%itype(l) == istsoil  .or. patch%itype(i) == nc3crop .or. patch%itype(i) == nc3irrig_dri .or. patch%itype(i) == nc3irrig_spr .or. patch%itype(i) == nc3irrig_fld)then
                      if ( present(num_reseed_patch) ) then

...
contains
  ...
  subroutine calc_plant_nitrogen_demand(this, bounds,  num_soilp, filter_soilp, &
    ...
    use pftconMod              , only : ntmp_soybean, nirrig_dri_tmp_soybean, nirrig_spr_tmp_soybean, nirrig_fld_tmp_soybean
    use pftconMod              , only : ntrp_soybean, nirrig_dri_trp_soybean, nirrig_spr_trp_soybean, nirrig_fld_trp_soybean
               ...
               if (leafout(p) >= huileaf(p) .and. hui(p) < huigrain(p)) then
                  ...
                  !they all seemed to be going through the retranslocation loop for soybean - good news.
                   ...
                   if (astem(p) == astemf(ivt(p)) .or. &
                       (ivt(p) /= ntmp_soybean .and. ivt(p) /= nirrig_dri_tmp_soybean .and. ivt(p) /= nirrig_spr_tmp_soybean .and. ivt(p) /= nirrig_fld_tmp_soybean .and.&
                       ivt(p) /= ntrp_soybean .and. ivt(p) /= nirrig_dri_trp_soybean .and. ivt(p) /= nirrig_spr_trp_soybean .and. ivt(p) /= nirrig_fld_trp_soybean)) then
